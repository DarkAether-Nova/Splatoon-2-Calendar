// ===============================
// SALMON RUN - ROTACIONES Y GESTIÓN UNIFICADA
// ===============================
...
let isCurrentSalmonOpen = true;
let storedSalmonOpenTime = "";
...
// ===============================
// ACTUALIZAR SALMON RUN DINÁMICO
// ===============================
...
function updateSalmonRunDynamic() {
    ...
    // Gestionar siguientes rotaciones
    for (let i = 0; i < 3; i++) {
    ...
    // Temporizador principal de cierre
    if (currentEvent) {
        ...
        if (diff > 0) {
            ...
            let remainingStr = "Restante: ";
            if (days > 0) remainingStr += `${days}d `;
            remainingStr += `${hours}h ${mins}m ${secs}s`;
    ...
    // Temporizadores para las siguientes rotaciones
    for (let i = 0; i < 3; i++) {
        const itemIdx = currentEvent ? i : i + 1;
        const nextItem = nextEvents[itemIdx];
